...
class PosenetActivity :
  ...
  private val minConfidence = 0.25
  ...
  private var physioOutput: Int? = 0
  private var physio: Physio? = null
  private val EXERCISE_MODES = 3
  private var exerciseMode = 0
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    physio = Physio(this.context!!)

    surfaceView?.setOnClickListener { view ->
      exerciseMode = (exerciseMode + 1) % EXERCISE_MODES

      if (exerciseMode == 0){
        exerciseName = "Squat"
        physio!!.start()
      }else if (exerciseMode == 1){
        exerciseName = "Right Arm Flexion"
        physio!!.start()
      }else {
        exerciseName = "Arms Raise"
        physio!!.start()
      }
  ...
  private fun draw(canvas: Canvas, person: Person, bitmap: Bitmap) {
    ...
    if (exerciseMode == 1) {
      canvas.drawText(
              "Flexion Angle: %d".format(physioOutput),
              (15.0f * widthRatio),
      ...
      canvas.drawText(
              "Repetition count: %d".format(physioOutput),
              (15.0f * widthRatio),
              (50.0f * heightRatio + bottom),
              paint
      )
  ...
  private fun processImage(bitmap: Bitmap) {
    ...
    if (exerciseMode == 0) {
      physioOutput = physio?.OnSquat(person)
    }else if (exerciseMode == 1){
      physioOutput = physio?.OnFlexion(person)
    }else{
      physioOutput = physio?.OnArmRaise(person)
    }
